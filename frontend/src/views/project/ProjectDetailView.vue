<template>
  <div class="flex min-h-screen bg-transparent py-8 justify-center">
    <Card class="w-full">
      ...
      <template #content>
        <div
          class="w-full grid grid-cols-1 md:grid-cols-2 gap-4 bg-blue-100 rounded-lg p-5 text-blue-600"
        >
      ...
      <template #footer>
        <div class="w-full flex justify-end">
          <Button
            @click="sendStatus"
            class="btn-submit p-2 rounded-md"
            v-if="project?.status == 'propose' && authStore.user?.role == 'teacher'"
            >Gửi duyệt</Button
          >
          <Button
            @click="Approve"
            class="btn-submit p-2 rounded-md"
            v-if="project?.status == 'pending' && authStore.user?.role == 'admin'"
            >Duyệt đề tài</Button
          >
        </div>
      </template>
