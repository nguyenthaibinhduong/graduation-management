    ...
    <Card class="w-full mb-6 shadow-lg">
      ...
      <template #content>
        ...
        <DataTable
          ...
          class="p-datatable-sm w-full rounded-lg shadow"
          stripedRows
        >
              ...
              >
                <div v-if="studentScores[selectedStudentId]?.[role]?.length">
                  <DataTableCustom
                    ...
                    :data="studentScores[selectedStudentId][role]"
                    :columns="[
                  ...
                  <div class="flex justify-end mb-2">
                    <Button
                      v-if="studentScores[selectedStudentId][role]?.length"
                      label="Mở khóa điểm"
                      icon="pi pi-unlock"
                      size="small"
                      severity="warning"
                      @click="unlockAllScoreDetails(studentScores[selectedStudentId][role])"
                      class="mb-2 mt-2"
                    />
                  </div>
                </div>
...
import { Card, DataTable, Column, Tag, Divider, Button, Dialog, TabView, TabPanel } from 'primevue'
...
const unlockAllScoreDetails = async (scoreDetails) => {
  if (!scoreDetails?.length) return
  for (const detail of scoreDetails) {
    if (detail?.id) {
      await scoreStore.unlockScoreDetail(detail.id)
    }
  }
}
</script>
