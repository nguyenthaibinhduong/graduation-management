      ...
      >
        <template v-slot:body="{ data }">
          ...
          <span v-else-if="col.type === 'image'">
            <img
              class="rounded-xl"
              v-if="getNestedValue(data, col.field) !== 'N/A'"
              :src="getNestedValue(data, col.field)"
              alt="image"
          ...
          <span v-else-if="col.type === 'status'">
            <template v-if="col.statuses">
              <span
                v-if="getStatus(col.statuses, getNestedValue(data, col.field))"
                :class="`px-2 py-1 text-sm rounded-xl ${
                  getStatus(col.statuses, getNestedValue(data, col.field)).class
                }`"
              >
          ...
          <template #body="{ data }">
            <div class="relative">
              ...
              <Button
                ...
                class="w-[30px] h-[30px] ms-2 bg-transparent border-none text-black"
              ...
              <OverlayPanel ref="dropdown">
                <div class="w-[100px] text-sm">
                  <button
                    class="w-full px-3 py-2 hover:bg-gray-100 flex items-center"
                    @click="$emit('edit', selectedItem)"
                  >
                    <i class="pi pi-pencil mr-2"></i> Sửa
                  </button>
                  <button
                    class="w-full px-3 py-2 hover:bg-gray-100 flex items-center text-red-500"
