...
export function createGroupStore(entity) {
  return defineStore(entity, () => {
    ...
    const assignReviewer = async (body) => {
      try {
        const { data } = await api.post('/groups/assign-reviewer', body)
        showToast('Gán giáo viên phản biện thành công!', 'success')
        await fetchItems()
        return data
      } catch (error) {
        showToast(error?.response?.data?.message || 'Gán phản biện thất bại!', 'error')
        throw error
      }
    }

    const assignCommittee = async (body) => {
      try {
        const { data } = await api.post('/groups/assign-committee', body)
        showToast('Gán hội đồng thành công!', 'success')
        await fetchItems()
        return data
      } catch (error) {
        showToast(error?.response?.data?.message || 'Gán hội đồng thất bại!', 'error')
        throw error
      }
    }
