...
export class UserFactory {
  static async create(role?: UserRole): Promise<Partial<User>> {
    return {
      ...
      role: role ?? faker.helpers.arrayElement([UserRole.STUDENT, UserRole.TEACHER, UserRole.ADMIN]),
      created_at: new Date(),
      updated_at: new Date(),
    };
  }
  ...
  static async createMany(count: number): Promise<Partial<User>[]> {
    const roles = [...Array(5).fill(UserRole.STUDENT), ...Array(5).fill(UserRole.TEACHER), ...Array(count - 10).fill(UserRole.ADMIN)];
